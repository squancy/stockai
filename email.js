...
if (localStorage.getItem('stock_email') == 'no' || !localStorage.getItem(
    'stock_email')) {
  ...
  _('emailText').innerHTML = `
      <p style="text-align: center;">
        Want to get notifications about your stocks? Enter your email address below!
      </p>
    `;
  ...
  _('innerBtn').addEventListener('click', async function handler(e) {
    ...
    try {
      ...
      if (final == "success") {
        _('status').innerHTML = `
                  <p class="green">
                    Email address has been successfully submitted.
                  </p>
                `;
  ...
  _('emailText').innerHTML = `
      <p style="text-align: center;" id="unsub">
        <a href="#">Unsubscribe from newsletter</a>
      </p>`;
  _('unsub').addEventListener('click', async function unsubscribe(e) {
    ...
    try {
      ...
      if (final == "success") {
        ...
        _('emailText').innerHTML = `
                  <p class="red" style="text-align: center;">${final}</p>
                `;
      }
    } catch (e) {
      _('emailText').innerHTML = `
